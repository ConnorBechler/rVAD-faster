...
def snre_highenergy(fdata, nfr10, flen, fsh10, ENERGYFLOOR, pv01, pvblk):
     ...
     for i in range(1, int(np.floor(nfr10/NESEG))+1):
          ...
          emin[(i-1)*NESEG+1: i*NESEG+1] = eY[int(np.floor(NESEG*0.1))]
          if i !=  1:
     ...
     if i*NESEG !=  nfr10:
          ...
          emin[i*NESEG+1:nfr10+1] = eY[int(np.floor((nfr10-(i-1)*NESEG)*0.1))]
...
#      D = np.insert(D,0,'inf')
     ...
     for i in range(2, nfr10+1):
          postsnr[i] = max(np.log10(e[i])-np.log10(emin[i]), 0)
